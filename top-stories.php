...
Description: Create top stories from Facebook's likes, shares, comments and Twitter's tweets, Google+, Pinterest, VKontakte and Linkedin, found your top viral posts (most shared), display charts and show trending posts and authors. This is a free version of a complete plugin sold on CodeCanyon.
Version: 1.72
Author: Giulio Pons
	...
	<div class="wrap tss">
		<form action="options.php" method="post">
			...
			if($options['top_stories_days']==20000) {
				...
				<?php
				/*
					this iframe i used to load dynamically chart.php
				*/
				?>
				<iframe src="<?php echo plugin_dir_url( __FILE__ )?>chart.php?d=<?php echo $options['top_stories_days']?>" width="92.5%" height="300"></iframe>
				<?php
			}
			?>
			<div class='statblock' style='padding-bottom:30px'>
				...
				<?php
				/*
					this iframe i used to load dynamically piechart.php
				*/
				?>
				<iframe src="<?php echo plugin_dir_url( __FILE__ )?>piechart.php?d=<?php echo $options['top_stories_days']?>" width="100%" height="300"></iframe>
			</div>
			<div class='statblock last' style='padding-bottom:30px'>
				...
				<?php
				/*
					this iframe i used to load dynamically colchart.php
				*/
				?>
				<iframe src="<?php echo plugin_dir_url( __FILE__ )?>colchart.php?sn=all" width="100%" height="300"></iframe>
			</div>
		...
		<div class="updated"><p>This is the <b>FREE VERSION</b>, which is a limited version of the complete plugin. This limited version shows historic data only for 1 month. The complete version comes with support for <code>shortcodes</code>, top authors widget, charts for single posts and for all your blog, email alerts for authors, complete social history for each post and globally for your site and multilingual support.<br> <a href="http://codecanyon.net/item/top-social-stories-plugin-and-widget/5888553?ref=ginoplusio" target="_blank">Upgrade to complete version NOW for only 18$!</a></p></div>
						...
						<tr>
							...
							<td>
								<fieldset>
									...
									<br />
									<span class="description"><?php _e("Top Stories plugin will send an email notice to the author when its post will pass this number of social interactions. Put 0 to stop sending emails.","top-stories-plugin");?> <b>AVAILABLE ONLY IN COMPLETE VERSION</b><br/>
									</span>
								</fieldset>
							</td>
						</tr>
...
function top_stories_activate() {
	...
	*/
	$rs = $wpdb->get_results("se"."lect COLUMN_NAME,COLUMN_TYPE f"."ro"."m information_schema.COLUMNS where TABLE_NAME='".$wpdb->prefix."top_stories_old' and COLUMN_TYPE LIKE '%unsigned%'", OBJECT);
	if ($rs) {
		foreach ($rs as $row) {
			if($row->COLUMN_NAME!="id_post") {
				$wpdb->query("ALTER TABLE  `".$wpdb->prefix."top_stories_old` CHANGE  `".$row->COLUMN_NAME."`  `".$row->COLUMN_NAME."` INT NOT NULL;");
			}
		}
	}
